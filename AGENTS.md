...
- When uncertain about specific AWS details, API parameters, permissions, limits, or error codes, verify against documentation rather than guessing. State uncertainty explicitly if you cannot confirm.
...
- MUST NOT hit the Secrets Manager Agent daemon directly.
- MUST use `{{resolve:secretsmanager:secret-id:SecretString:json-key}}` with `asm-exec` so the secret resolves at runtime without entering context.
