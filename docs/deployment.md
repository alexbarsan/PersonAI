...
Deploy one task revision and monitor `dreamlens.async_jobs.*` metrics and task logs. The worker considers only completed dreams whose owners currently consent to AI processing and history use. It skips dreams already embedded with the active model, dimensions, and version; stale provider versions are replaced safely. Reset `Jobs__EmbeddingBackfill__Enabled` to `false` after the run.
