...
3. **Exploration and validation**: Map normalization/provenance and observations, Ask recovery/source UX, onboarding/auth route handling, responsive/accessibility and native validation inventory. Map provenance completed 2026-09-15: each new fact retains its normalized form version, source output field, source schema, and extraction confidence; an owner can open the contributing journal entries from the Map. Existing facts intentionally retain an `unknown` source field until historical re-extraction is explicitly scheduled. Ask recovery/source UX completed 2026-09-15: the client obtains owner-scoped active-index readiness before asking, shows pending indexing rather than consuming a question, and renders named ranked citations with direct journal links after an answer.
