...
| S24 | Voice capture and transcription | Complete in code; live enablement is pending ECS task-definition Terraform drift reconciliation. | Added a provider abstraction, Amazon Transcribe adapter, multipart upload policy, private S3 input/output handling, Premium gating, daily cap, byte/type/duration checks, owner-scoped polling, default deletion, explicit retention, SQS worker, EF migration, and per-operation cost/latency ledger rows. Terraform declares a controlled dev enablement; QA/prod remain disabled. | Added record/review/transcribe flow with microphone permission, an explicit default-off retention switch, processing feedback, and transcript insertion for user review before interpretation. |
