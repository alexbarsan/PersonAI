...
| S32 | Async assets foundation | Worker execution, signed access, and application queue publishing remain. | Added private S3 asset bucket/module, KMS-encrypted SQS queue with DLQ, long polling, scoped ECS permissions, and idempotent `AsyncJobs` persistence. Remaining: signed access service, queue publisher/consumer, worker host, retry telemetry, and backfill processing. | Add shared job status client patterns that S25, exports, and future long-running features can reuse. |
