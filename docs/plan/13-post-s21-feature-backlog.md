...
| S32 | Async assets foundation | Concrete image/export handlers and embedding backfill remain. | Added private S3 asset bucket/module, KMS-encrypted SQS queue with DLQ, long polling, scoped ECS permissions, idempotent `AsyncJobs` persistence, publisher, hosted worker, embedding handler, job-status endpoint, presigned S3 asset service, and retry/lease/duration metrics. | Add shared job status client patterns that S25, exports, and future long-running features can reuse. |
