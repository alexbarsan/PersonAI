...
| S32 | Async assets foundation | Worker execution, job status APIs, and concrete embedding/image/export handlers remain. | Added private S3 asset bucket/module, KMS-encrypted SQS queue with DLQ, long polling, scoped ECS permissions, idempotent `AsyncJobs` persistence, SQS publisher, and presigned S3 asset service. Remaining: queue consumer/worker, retry telemetry, and backfill processing. | Add shared job status client patterns that S25, exports, and future long-running features can reuse. |
