...
| D19 | Premium interpretation and cognitive analysis | Route Premium dream submissions directly to `deepseek-v4-pro`; do not first call the Free model. Keep the dedicated `deep-dream-interpreter` persona as an optional, persisted Cognitive Analysis that uses owner-scoped profile data and the most relevant consent-allowed dream summaries retrieved through pgvector. Gate Cognitive Analysis by Premium entitlement and daily quota, and record a cost/latency ledger row for every call. | Premium receives one stronger primary interpretation rather than two sequential interpretations. Cognitive Analysis remains a clearly different, bounded second lens without sending full journal history. |
| D20 | Dream-image generation | Compose and encrypt a bounded symbolic prompt before queueing. Snapshot style, tier, provider, model, dimensions, quality, and estimated cost on the image request. Use the provider registry to route Free to OpenAI `gpt-image-1-mini` Low and Premium to the same model at Medium quality. | Preserves reproducibility and cost attribution while allowing future model or tier changes without altering queued work. |
...
- **Dream-image requests**: compose a bounded, symbolic image prompt from the completed interpretation before enqueueing work. Encrypt and persist the prompt, prompt version, visual style, entitlement tier, provider, model, dimensions, and estimated cost with the image request. Workers must use this immutable snapshot rather than current configuration, so retries are reproducible and model changes do not alter a queued request.
- **Image-provider routing**: resolve a model route independently for Free and Premium tiers through a provider registry. Provider adapters receive the persisted route and implement only their provider contract. Keep both routes disabled until a supported text-to-image provider and explicit cost/quota policy are selected; Free may later use a lower-cost route without changing the request, job, asset, or ledger flow.
