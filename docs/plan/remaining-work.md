...
- Embeddings use PostgreSQL `pgvector` and 1,024-dimensional Amazon Nova Multimodal Embeddings, not full-history prompts. S35 code and Terraform are complete; dev invocation, apply, backfill, and live retrieval verification remain.
- Historical fact backfill and semantic clustering: the Dream DNA overview is implemented, while semantic similarity needs the S35 dev backfill and clustering remains future work.
- S28 Ask Dream DNA is implemented with owner-scoped semantic retrieval, safety/schema validation, quotas, cost rows, evidence links, and UI. Live answers require the Nova dev backfill and controlled verification.
...
- `pgvector`, SQS job wiring, owner-scoped retrieval, and the embedding handler are implemented. S35 replaces the blocked Titan provider with Nova Multimodal Embeddings, filters retrieval to active vector metadata, replaces stale rows during backfill, and adds background embedding cost/latency ledger entries. Cloud rollout and live backfill are still pending.
