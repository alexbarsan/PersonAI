...
- Dream-image generation remains disabled in dev: Nova Canvas is legacy and Bedrock rejects this account because it was not actively used before the legacy cutoff. The request pipeline now encrypts and stores a versioned prompt plus an immutable Free/Premium provider-route snapshot before queueing. Select and implement a supported replacement, configure per-tier quotas and costs, then enable the applicable route.
