...
- Dream-image generation remains request-only and Premium-only in the product surface, but its dev runtime is disabled: Nova Canvas is legacy and Bedrock rejects this account because it was not actively used before the legacy cutoff. The account has no active Bedrock text-to-image model. Select and implement a supported replacement before QA, production, or public launch, then configure an explicit image quota and cost profile.
...
- A private KMS-encrypted S3 asset bucket and signed-access service are implemented. Voice input is private and deleted after transcription by default; explicit retention exposes it only through a short-lived signed URL. The future local-first client backup, Free device transcription, retry outbox, and tier-specific retention policy are tracked as S33. The request-only Premium dream-image handler, result UI, ledger pricing, and dev Nova Canvas configuration are implemented, but Nova Canvas is disabled after Bedrock rejected this account under its legacy policy. Replace it with a supported text-to-image provider before enabling the flow again; export job generation remains future work.
