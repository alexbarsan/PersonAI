...
- Embeddings use PostgreSQL `pgvector` and 1,024-dimensional Amazon Nova Multimodal Embeddings, not full-history prompts. S35 is deployed in dev and all 3 eligible historical dreams were backfilled; an authenticated Similar Dreams and Ask Dream DNA response smoke test remains.
...
- `pgvector`, SQS job wiring, owner-scoped retrieval, and the embedding handler are live in dev. S35 replaced the blocked Titan provider with Nova Multimodal Embeddings, filters retrieval to active vector metadata, replaces stale rows during backfill, and adds background embedding cost/latency ledger entries. The bounded dev backfill completed all 3 eligible jobs with 3 ledger rows and empty main/DLQ queues; the authenticated endpoint smoke test remains.
