...
- Dream-image generation is request-only and Premium-only in dev, using Nova Canvas at an estimated `$0.04` for each completed standard 1024x1024 image. Nova Canvas is legacy with an AWS-published EOL of 2026-09-30, so select and implement a supported replacement before QA, production, or public launch. Add an explicit image quota before scaling access.
